 ...
 */
function resolveCallbackChatId(ctx, fallbackUserId) {
 ...
 */
async function registerChannelOnBotJoin(ctx, chatId, isChannel, notificationService) {
    try {
        const chat = await ctx.getChat(chatId);
        channelRegistry_1.channelRegistry.saveChannel(chatId, { title: chat.title, type: chat.type });
        await notificationService.notifyAdmin(`✅ Bot added to channel: ${chat.title ?? '—'} (ID: ${chatId})`);
    }
    catch (e) {
        ...
        await notificationService.notifyAdmin(`✅ Bot added to channel: — (ID: ${chatId})`);
    }
}
/**
 * Удаляет чат из реестра и уведомляет администратора (один раз, если запись была).
 */
async function unregisterChannelOnBotLeave(chatId, notificationService) {
    const removed = channelRegistry_1.channelRegistry.removeChannel(chatId);
    if (!removed) {
        return;
    }
    await notificationService.notifyAdmin(`❌ Bot removed from channel: ${removed.title ?? '—'} (ID: ${chatId})`);
}
function registerEventHandlers(bot) {
    ...
    bot.on('bot_added', async (ctx) => {
        ...
        logger_1.logger.info(`bot_added: chat_id=${chat_id}`);
    ...
    bot.on('user_added', async (ctx) => {
        ...
        if (channelRegistry_1.channelRegistry.getChannel(chat_id) !== null) {
            return;
        }
        logger_1.logger.info(`user_added (self): chat_id=${chat_id}`);
        await registerChannelOnBotJoin(ctx, chat_id, isChannel, notificationService);
    });
    /**
    ...
    bot.on('bot_started', async (ctx) => {
        ...
        if (payload) {
            ...
            if (parsed?.type === 'post') {
                stateManager_1.stateManager.setState(chatId, user.user_id, {
                    mode: 'commenting',
                    postId: parsed.id,
                    createdAt: new Date(),
                });
    ...
    bot.on('message_created', async (ctx) => {
        ...
        if (text === '/status') {
            ...
            const comments = commentService_1.commentService.countByChatId(chatId);
    ...
    bot.on('message_callback', async (ctx) => {
        ...
        logger_1.logger.info(`message_callback: payload=${callbackData}, chat=${chatId}`);
