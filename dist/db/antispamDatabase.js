...
exports.ANTISPAM_DB_PATH = void 0;
exports.getAntispamDb = getAntispamDb;
exports.getAntispamDbMeta = getAntispamDbMeta;
exports.setAntispamDbMeta = setAntispamDbMeta;
exports.closeAntispamDb = closeAntispamDb;
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const better_sqlite3_1 = __importDefault(require("better-sqlite3"));
const DATA_DIR = node_path_1.default.resolve(__dirname, '../../data');
exports.ANTISPAM_DB_PATH = node_path_1.default.join(DATA_DIR, 'antispam.db');
let antispamDb = null;
function getAntispamDb() {
    if (antispamDb) {
        return antispamDb;
    }
    node_fs_1.default.mkdirSync(DATA_DIR, { recursive: true });
    const instance = new better_sqlite3_1.default(exports.ANTISPAM_DB_PATH);
...
function initAntispamSchema(db) {
    db.exec(`
...
function getAntispamDbMeta(key) {
    const row = getAntispamDb()
        .prepare('SELECT value FROM antispam_db_meta WHERE key = ?')
        .get(key);
    return row?.value ?? null;
}
function setAntispamDbMeta(key, value) {
    getAntispamDb()
        .prepare(`INSERT INTO antispam_db_meta (key, value) VALUES (?, ?)
       ON CONFLICT(key) DO UPDATE SET value = excluded.value`)
        .run(key, value);
}
function closeAntispamDb() {
    if (!antispamDb) {
        return;
    }
    antispamDb.close();
...
//# sourceMappingURL=antispamDatabase.js.map