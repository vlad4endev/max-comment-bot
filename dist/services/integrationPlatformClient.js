...
function normalizeVkGroupLookup(input) {
    ...
    if (/^club[a-zA-Z_]/i.test(lookup))
        return lookup.slice(4);
    if (/^public[a-zA-Z_]/i.test(lookup))
        return lookup.slice(6);
    return lookup;
}
/** VK API 5.139+: response — объект { groups, profiles }; раньше — массив. */
function parseVkGroupsGetByIdItems(response) {
    if (!response)
        return [];
...
async function fetchVkGroupByLookup(token, lookup) {
    if (!lookup) {
    ...
    try {
        ...
        if (data.error) {
            ...
            logger_1.logger.debug('fetchVkGroupByLookup vk error', { lookup, error: data.error });
        ...
        logger_1.logger.debug('fetchVkGroupByLookup failed', { lookup, err });
