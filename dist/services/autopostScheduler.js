...
async function processDuePost(post, token) {
    try {
        const result = await (0, autopostTelegramSender_1.sendAutopostToTelegram)(token, post);
        if (result.warning) {
        ...
        logger_1.logger.error('autopostScheduler: send failed', { id: post.id, error: message });
    }
}
async function tick() {
    if (ticking) {
