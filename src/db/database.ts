...
function migrateChannelJoinNotifiedSchema(database: Database.Database): void {
  const tgCols = database.prepare('PRAGMA table_info(tg_channels)').all() as { name: string }[]
  if (!tgCols.some((c) => c.name === 'admin_join_notified')) {
    database.exec(
      'ALTER TABLE tg_channels ADD COLUMN admin_join_notified INTEGER NOT NULL DEFAULT 0',
    )
    database.exec('UPDATE tg_channels SET admin_join_notified = 1 WHERE bot_is_admin = 1')
  }

  const maxCols = database.prepare('PRAGMA table_info(channels)').all() as { name: string }[]
  if (!maxCols.some((c) => c.name === 'admin_join_notified')) {
    database.exec(
      'ALTER TABLE channels ADD COLUMN admin_join_notified INTEGER NOT NULL DEFAULT 0',
    )
...
function migrateCommentSyncSchema(database: Database.Database): void {
  ...
  const commentCols = database.prepare('PRAGMA table_info(comments)').all() as Array<{ name: string }>
  ...
  const mappingCols = database.prepare('PRAGMA table_info(post_comment_mapping)').all() as Array<{
    name: string
  }>
  ...
  database.prepare(
    `CREATE INDEX IF NOT EXISTS idx_post_comment_mapping_tg
     ON post_comment_mapping (chain_id, tg_msg_id)`,
  ).run()
  database.prepare(
